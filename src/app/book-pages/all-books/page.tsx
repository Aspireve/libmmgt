...
const BooksPage = () => {
  const { data, isLoading } = useList<BookData>({ resource: "book/all" });
  ...
  const confirmDelete = () => {
    if (selectedBookId) {
      mutate(
        ...
        {
          onError: () => {
            toast.error("There was an issue deleting the book");
          },
          onSuccess: () => {
      ...
      cell: ({ row }) => (
        <div className="flex gap-2">
          ...
          <Button
            variant="ghost"
            size="icon"
            onClick={() => handleDeleteClick(row.original.book_id)}
          >
        ...
        <div className="container">
          <div className="grid grid-cols-[30%_70%] p-4">
            <div className="flex items-center gap-[10px]">
              ...
              <p className="bg-[#F9F5FF] rounded-2xl text-[#6941C6]">
                {isLoading ? (
            ...
            <div className="flex items-center justify-end py-4 gap-3">
              ...
              <Link href={"/book-pages/add-book"}>
                <Button className="shadow-none border border-[#989CA4] rounded-[8px] text-[#BBBBBB] flex items-center px-4 py-2">
